...
def test_training_server_models_list():
    ...
    for gated_name in ["ttft_gated", "tpot_gated"]:
        ...
        print(
            f"Model {gated_name}: exists={models[gated_name]['exists']}, size={models[gated_name]['size_bytes']} bytes"
        )
...
def test_prediction_via_prediction_server():
    ...
    data = r.json()
    required_fields = ["ttft_ms", "tpot_ms", "predicted_at", "model_type", "objective_type", "last_model_load"]
...
def test_invalid_pod_type():
    ...
    if r.status_code == 422:
        ...
        raise AssertionError(f"Unexpected status code {r.status_code} for invalid pod_type")
...
def test_tif_features_mean_learns_equation():
    ...
    pr = requests.post(f"{PREDICTION_URL}/predict/bulk/strict", json={"requests": test_cases}, timeout=30)
    assert pr.status_code == 200, f"Predict failed: {pr.status_code}"
...
def test_tif_features_quantile_learns_distribution():
    ...
    ttft_mu = 100.0 + ITL * 2.0 + NRW * 3.0 + NRR * 4.0 + KV * 50.0 + PCS * 30.0 + prefill_tif * TIF_COEFF_TTFT
    tpot_mu = 10.0 + KV * 100.0 + ITL * 0.5 + NTG * 1.0 + NRR * 5.0 + decode_tif * TIF_COEFF_TPOT
    ...
    pr = requests.post(f"{PREDICTION_URL}/predict/bulk/strict", json={"requests": mono_cases}, timeout=30)
    ...
    for bucket_label, tif_lo, tif_hi in [("low-TIF", 0, 2_000), ("high-TIF", 8_000, 10_000)]:
        ...
        ttft_mu_t = 100.0 + ITL * 2.0 + NRW * 3.0 + NRR * 4.0 + KV * 50.0 + PCS * 30.0 + pf_tif * TIF_COEFF_TTFT
        tpot_mu_t = 10.0 + KV * 100.0 + ITL * 0.5 + NTG * 1.0 + NRR * 5.0 + dc_tif * TIF_COEFF_TPOT
        ...
        pr2 = requests.post(f"{PREDICTION_URL}/predict/bulk/strict", json={"requests": test_cases}, timeout=60)
        ...
        print(
            f"  [{bucket_label}] coverage: TTFT={ttft_cov:.3f}, TPOT={tpot_cov:.3f} "
...
def test_training_server_flush_api():
    ...
    assert flush_response["success"] is True
    assert flush_response["metrics_cleared"] is False
    ...
    assert flush_all_response["success"] is True
    assert flush_all_response["metrics_cleared"] is True
    ...
    assert default_flush_response["success"] is True
...
def test_training_server_flush_error_handling():
    ...
    assert r.status_code == 200
    response = r.json()
    assert response["metrics_cleared"] is True
